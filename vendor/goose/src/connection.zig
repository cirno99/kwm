...
/// How long (in ms) `call` waits for a reply before giving up.
/// Prevents a non-responsive D-Bus peer from blocking the caller forever.
const call_timeout_ms: i32 = 3000;
    fn call(self: *Connection, data: []u8, serial: u32) !core.Message {
        ...
        while (true) {
            ...
            // Read new message (with a poll timeout so a stuck peer does not
            // block this thread forever)
            var pfd = [_]posix.pollfd{.{ .fd = self.__inner_sock.socket.handle, .events = posix.POLL.IN, .revents = 0 }};
            const n = posix.poll(pfd[0..], call_timeout_ms) catch |err| {
                std.debug.print("goose: poll failed: {}\n", .{err});
                return error.PollFailed;
            };
            if (n == 0) {
                std.debug.print("goose: call timed out after {} ms\n", .{call_timeout_ms});
                return error.CallTimeout;
            }
