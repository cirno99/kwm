...
const Tag = union(enum) {
    ...
    pub fn of(self: *const @This(), base: union(enum) { output: *Output, window: *Window }) u32 {
        ...
        return if (ctx.cfg.bar.tags) |area|
            switch (self.*) {
                .tag => |tag| tag,
...
pub const Action = union(enum) {
    quit: struct { exit_session: bool, hook: ?[]const []const u8 = null },
    close,
    spawn: struct {
        argv: []const []const u8,
    },
    ...
    modify_scroller_row_mfact: struct { change: SetStep },
    modify_scroller_mode: struct { mode: layout.Scroller.Mode },
    canvas_pan: struct { dx: i32, dy: i32 },
    canvas_focus_direction: struct { direction: types.WindowDirection },
    canvas_center_focus,
    canvas_flex_layout,
